...
void GroundStateProblem::setup() {
    ...
    if (!exists(compilepath)) {
        ...
        for (int i = 0; i < L; i++) {
            ...
            string Efuncname = "E_" + to_string(i) + "_" + to_string(n);
            SXFunction Ef = SXFunction(Efuncname, nlpIn("x", x, "p", p), nlpOut("f", E));
            Ef.generate();
            Function Egradf = Ef.gradient();
            string Egradfuncname = "Egrad_" + to_string(i) + "_" + to_string(n);
            Egradf.setOption("name", Egradfuncname);
            Egradf.generate();
...
GroundStateProblem::GroundStateProblem() {
    ...
    for (int i = 0; i < L; i++) {
        for (int n = 0; n <= nmax; n++) {
            ...
            string Efuncname = "E_" + to_string(i) + "_" + to_string(n);
            string Egradfuncname = "Egrad_" + to_string(i) + "_" + to_string(n);
            Ef[j] = ExternalFunction(Efuncname);
            Egradf[j] = ExternalFunction(Egradfuncname);
...
//        Ef[i].getOutput(Ei, NL_F);
        Ei = Ef[i](vector<DMatrix>{f, params})[0].toScalar();
        E += Ei;
    }
    if (!grad.empty()) {
...
//            Egradf[i].output().getArray(gradi.data(), gradi.size(), DENSE);
            DMatrix gradi = Egradf[i](vector<DMatrix>{f, params})[0];
            for (int j = 0; j < 2 * L * dim; j++) {
                grad[j] += gradi[j].toScalar();
            }
        }
    }
